...
import com.polystorage.dao.ChiTietDonXuatDAOImpl;
 ...
 * @author xPeter
 */
public class FrameThemDonXuat extends javax.swing.JFrame {

    /**
     * Creates new form FrameUpdate
     */
    public FrameThemDonXuat(DonXuat donXuat) {
        initComponents();
        setLocationRelativeTo(null);
        this.fillToKhachHang();
        this.fillToSanPham();
        this.fillToNhanVien();
        this.fillToKho();
        if (donXuat != null) {
            ...
            maDX = donXuat.getMaDX();
            this.donXuat = donXuat;
            isAddDonNhap = true;
            this.showDetailDonXuat();
        } else {
            maDX = donXuatDAOImpl.getNextId();
            txtMaDX.setText(String.valueOf(maDX));
            txtNgayXuat.setDate(new Date());
        }
        this.fillToChiTietDonXuat();
        this.updateUI();
    }
    ...
    private void initComponents() {
        ...
        jLabel3.setText("Mã Đơn Xuất:");

        jLabel4.setFont(new java.awt.Font("Tahoma", 0, 15)); // NOI18N
        jLabel4.setText("Nhân Viên:");

        txtMaDX.setEditable(false);
        txtMaDX.setFont(new java.awt.Font("Tahoma", 0, 15)); // NOI18N
        ...
        cboKhachHang.setFont(new java.awt.Font("Tahoma", 0, 15)); // NOI18N
        ...
        txtNgayXuat.setFont(new java.awt.Font("Tahoma", 0, 15)); // NOI18N
            ...
            .addGroup(pnlCenterLayout.createSequentialGroup()
                ...
                .addGroup(pnlCenterLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(pnlCenterLayout.createSequentialGroup()
                        ...
                        .addGroup(pnlCenterLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addGroup(pnlCenterLayout.createSequentialGroup()
                                .addComponent(cboKhachHang, javax.swing.GroupLayout.PREFERRED_SIZE, 500, javax.swing.GroupLayout.PREFERRED_SIZE)
                            ...
                            .addGroup(pnlCenterLayout.createSequentialGroup()
                                .addGroup(pnlCenterLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING, false)
                                    ...
                                    .addComponent(txtMaDX, javax.swing.GroupLayout.Alignment.LEADING, javax.swing.GroupLayout.DEFAULT_SIZE, 200, Short.MAX_VALUE))
                                .addGap(36, 36, 36)
        ...
        lblXacNhan.setForeground(new java.awt.Color(255, 0, 51));
    ...
    private void btnThemSanPhamActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnThemSanPhamActionPerformed
        ...
        if (loi.equals("")) {
            if (isAddDonNhap == false) {
                donXuat = this.getDonXuat();
                if (donXuatDAOImpl.insertDonXuat(donXuat)) {
            ...
            boolean ok = true;
            ChiTietDonXuat chiTietDonXuat = this.getChiTietDonXuat();
            for (ChiTietDonXuat x : listChiTietDonXuat) {
                if (chiTietDonXuat.getSanPham().getMaSp().equals(x.getSanPham().getMaSp())) {
                    ok = false;
                    break;
                } else {
                    ok = true;
            ...
            if (ok) {
                chiTietDonXuatDAOImpl.insertChiTietDonXuat(chiTietDonXuat);
                this.fillToChiTietDonXuat();
            } else {
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(FrameThemDonXuat.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
    ...
    ChiTietDonXuatDAOImpl chiTietDonXuatDAOImpl = new ChiTietDonXuatDAOImpl();
    DonXuatDAOImpl donXuatDAOImpl = new DonXuatDAOImpl();

    List<ChiTietDonXuat> listChiTietDonXuat;
    boolean capNhat = false;
    boolean isAddDonNhap = false; //kiem tra da add don nhap chua

    private void showDetailDonXuat() {
        DonXuat donXuat = donXuatDAOImpl.getInfoDonXuat(maDX);
        txtMaDX.setText(donXuat.getMaDX().toString());
        //txtGhiChu.setText(donXuat.getGhiChu());
        cboKhachHang.setSelectedItem("(" + donXuat.getKhachHang().getMaKH() + ") " + donXuat.getKhachHang().getHoTen());
        cboKho.setSelectedItem(donXuat.getKho().getMaKho());
        txtNgayXuat.setDate(donXuat.getNgayXuat());
        if (donXuat.isTrangThai()) {
    ...
    private void fillToKhachHang() {
        List<KhachHang> list = khachHangDAOImpl.getListKhachHang(null);
        DefaultComboBoxModel model = (DefaultComboBoxModel) cboKhachHang.getModel();
        model.removeAllElements();
        for (KhachHang x : list) {
            model.addElement("(" + x.getMaKH() + ") " + x.getHoTen());
        }
    ...
    private void fillToChiTietDonXuat() {
        listChiTietDonXuat = chiTietDonXuatDAOImpl.getListChiTietDonXuat(maDX);
        if (listChiTietDonXuat != null) {
    ...
    private DonXuat getDonXuat() {
        ...
        KhachHang khachHang = khachHangDAOImpl.getInfoKhachHang(kh.substring(1, kh.lastIndexOf(")")));
        NhanVien nhanVien = nhanVienDAOImpl.getInfoNhanVien("ADMIN");
        Date ngayNhap = txtNgayXuat.getDate();
        DonXuat donXuat = new DonXuat(khachHang, kho, nhanVien, ngayNhap, false);
        return donXuat;
    }

    private ChiTietDonXuat getChiTietDonXuat() {
        ...
        ChiTietDonXuatId chiTietDonXuatId = new ChiTietDonXuatId(maDX, maSP.substring(1, maSP.indexOf(")")));
        ChiTietDonXuat chiTietDonXuat = new ChiTietDonXuat(chiTietDonXuatId, donXuat, sanPham, soLuong);
        return chiTietDonXuat;
    }

    private void updateChiTietDonXuat() {
        chiTietDonXuatDAOImpl.deleteChiTietDonXuat(maDX);
        ...
        for (int i = 0; i < rows; i++) {
            ...
            ChiTietDonXuatId chiTietDonXuatId = new ChiTietDonXuatId(maDX, maSP);
            ChiTietDonXuat chiTietDonXuat = new ChiTietDonXuat(chiTietDonXuatId, donXuat, sanPham, soLuong);
            chiTietDonXuatDAOImpl.insertChiTietDonXuat(chiTietDonXuat);
        }
        this.fillToChiTietDonXuat();
    ...
    private void xacNhanDon() {
        boolean ok = true;
        String maKho = cboKho.getSelectedItem().toString();
        int check = JOptionPane.showConfirmDialog(this, "Xác nhận đơn hàng " + maDX + " vào kho " + maKho, "Bạn muốn xác nhận?", JOptionPane.YES_NO_OPTION);
        if (check == 0) {
            for (ChiTietDonXuat x : listChiTietDonXuat) {
                SanPham sanPham = x.getSanPham();
                Kho kho = khoDAOImpl.getInfoKho(maKho);
            ...
            if (ok) {
                donXuat.setTrangThai(true);
                donXuatDAOImpl.updateDonXuat(donXuat);
